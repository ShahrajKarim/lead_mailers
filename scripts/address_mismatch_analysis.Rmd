...
It's difficult to select inns within the property dataset - some street names may just have the term "Inn" and these are residential addreses. To deal with this, properties with the term "Inn" are only selected if they don't have a door number:
...
ggplot(hotel_property_data, aes(x = hotel_proportion)) +  
    geom_histogram(binwidth = 1, fill = "red", color = "black", alpha = 0.7) +  
    labs(title = "Distribution of Hotels in Leeds by LSOA",
         x = "Proportion of Properties (%)",
         y = "Number of LSOAs") +
       theme_minimal()
```
...
household_count <- household_count %>%
  ...
  select(lsoa21cd, property_count, total_number_of_households, mismatched_addr, remaining_addr)
...
## Distribution of address mismatch
...
mismatch_old_plot <- ggplot(household_count, aes(x = mismatch_old)) +  
    ...
    scale_x_continuous(limits = c(-100, 100)) +  # Set X-axis range from 0 to 100
...
- These two factors alone account for **55%** of the mismatch between households and properties in Leeds.
